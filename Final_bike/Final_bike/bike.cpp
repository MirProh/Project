...
Bike::Bike()
{
	...
	rear.radius = 20;
	front.radius = 20;
	rear.b.pos = toGlobal({ -40,-10 });//положение колёс относительно корпуса
	...
	rear.s.p1 = { -30,0 };//крепления рессор к корпусу
	front.s.p1 = { 30,0 };
	rear.s.p2 = &rear;
	front.s.p2 = &front;
...
void Bike::update(int controls)
{
	...
	*this = rk(this);
    if ((rear.b.pos.x + rear.radius > 500)||(front.b.pos.x + front.radius < -500)) {
        double ground = -40;
        if (b.pos.y < ground) {
            b.pos.y = ground;
            b.momentum.y = 0;
        ...
        //условие взаимодействия заднего колеса с полом
        ...
        //условие взаимодействия переднего колеса с полом
        ...
        if (b.pos.y < b.pos.x / 2 + 210 ) {
            b.pos.y = b.pos.x / 2 + 210;
            b.momentum.y = 0;
            b.momentum.x *= 0.2;
        }
        //условие взаимодействия заднего колеса с полом
        if ((rear.b.pos.y - 1.5 * rear.radius < rear.b.pos.x/2 + 210)) {
            rear.b.pos.y = rear.radius + rear.b.pos.x/2 + 210;
            rear.b.momentum.y = 0;
            double vel = (-rear.b.momentum.x / rear.mass + rear.b.L * rear.radius) * 0.05;
            rear.b.momentum.x += vel;
            rear.b.L -= vel / rear.radius;
        ...
        //условие взаимодействия переднего колеса с полом
        if ((front.b.pos.y < front.b.pos.x / 2 + 1.5 * front.radius + 210)) {
            front.b.pos.y = front.b.pos.x / 2 + 1.5 * front.radius + 210;
            front.b.momentum.y = 0;
            //front.b.momentum.x *= 0.8;
            double vel = (-front.b.momentum.x / front.mass + front.b.L * front.radius) * 0.05;
            front.b.momentum.x += vel;
    ...
    //условие на не выход за границы экрана заднего колеса
    if (abs(rear.b.pos.x)+rear.radius>900) {
        rear.b.pos.x = rear.b.pos.x / abs(rear.b.pos.x) * (900- rear.radius);
        rear.b.momentum.x = -rear.b.momentum.x;
        //std::cout << rear.b.momentum.x << "\n";
    }
    //условие на не выход за границы экрана переднего колеса
	if (abs(front.b.pos.x) + front.radius > 900) {
		front.b.pos.x = front.b.pos.x / abs(front.b.pos.x) * (900 - front.radius);
		front.b.momentum.x = -front.b.momentum.x;

	}
    //делаем так чтобы все углы лежали в промежутке [-2pi;2pi]
...
//описание оператора сложения для байков, то есть состояний мотоцикла
...
//описание умножения состояния на константу
